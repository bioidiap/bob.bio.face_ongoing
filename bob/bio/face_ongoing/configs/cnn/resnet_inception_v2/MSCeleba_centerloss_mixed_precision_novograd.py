...
import pkg_resources  # needed when you run this script with bin/python
...
CHECKPOINT = "/idiap/temp/amohammadi/models/inception_v2_batchnorm_rgb_msceleba_mixed_precision_novograd"
...
LEARNING_RATE = 0.1
BATCH_SIZE = 616 * 2  # should be a multiple of 8
...
VALIDATION_BATCH_SIZE = 38  # should be 38 to match Tiago's old results
...
class CustomModel(tf.keras.Model):
    ...
    def train_step(self, data):
        ...
        self.train_learning_rate(self.optimizer.lr)
        return {
            m.name: m.result()
            for m in [self.train_loss, self.train_cross_entropy, self.train_center_loss, self.train_learning_rate]
        }
...
def build_and_compile_model(global_batch_size):
    ...
    train_learning_rate = tf.keras.metrics.Mean(name="learning_rate")
...
def train_and_evaluate():
    ...
    model = build_and_compile_model(global_batch_size=BATCH_SIZE)
    ...
    #     if epoch in range(5):  # LR warm-up
    #         return tf.linspace(0.032 / 25, 0.032, num=5)[epoch]
    #     elif epoch in range(5, 20):
    #         return 0.07
    ...
    callbacks = [
        ...
        CustomBackupAndRestore(backup_dir=f"{CHECKPOINT}/backup"),
    ]
