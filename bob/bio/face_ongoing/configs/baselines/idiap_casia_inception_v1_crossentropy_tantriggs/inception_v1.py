...
model_filename = inception_resnet_v1_casia_webface_crossentropy_tantriggs
...
# Extraction
#########
inputs = tf.placeholder(tf.float32, shape=(1, 160, 160, 4))

# Taking the embedding
prelogits = inception_resnet_v1(tf.stack([tf.image.per_image_standardization(i) for i in tf.unstack(inputs)]), mode=tf.estimator.ModeKeys.PREDICT)[0]
